CLASS zcl_pdakm_dummy_data DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  CONSTANTS:
      c_error                TYPE ZDAKM_IVIT-price VALUE '37707',
  ...
  PRIVATE SECTION.
  DATA:
      mt_partner         TYPE STANDARD TABLE OF ZDAKM_PTNR,
      mt_material        TYPE STANDARD TABLE OF ZDAKM_MTNR,
      mt_discount        TYPE STANDARD TABLE OF  ZDAKM_DISC,
      mt_head         TYPE STANDARD TABLE OF  ZDAKM_INHE,
      mt_item            TYPE STANDARD TABLE OF  ZDAKM_IVIT ,


      mo_random_partner  TYPE REF TO zcl_bs_dmo_random,
      mo_random_date     TYPE REF TO zcl_bs_dmo_random,
      mo_random_position TYPE REF TO zcl_bs_dmo_random,
      mo_random_material TYPE REF TO zcl_bs_dmo_random,
      mo_random_quantity TYPE REF TO zcl_bs_dmo_random.
    ...
    create_invoice
     ...
     create_head
        RETURNING
            VALUE(rs_result) TYPE ZDAKM_INHE,

       create_positions
             IMPORTING
             is_head  TYPE  ZDAKM_INHE.
ENDCLASS.



CLASS zcl_pdakm_dummy_data IMPLEMENTATION.
  METHOD create_discount.
   ...
   DELETE from zdakm_disc.

   INSERT zdakm_disc FROM TABLE @mt_discount .
  ENDMETHOD.
  ...
  DO id_count TIMES .
    ...
    DELETE FROM zdakm_inhe.
    INSERT zdakm_inhe FROM TABLE @mt_head.
      DELETE FROM zdakm_ivit.
    INSERT zdakm_ivit FROM TABLE @mt_item.
  ...
  DELETE from zdakm_mtnr .
  INSERT zdakm_mtnr FROM TABLE @mt_material .
  ENDMETHOD.
  ...
  mt_partner = VALUE #(
    ...
    DELETE from ZDAKM_PTNR .
     INSERT ZDAKM_PTNR FROM TABLE @mt_partner.
  ENDMETHOD.
  ...
  METHOD create_positions.
   IF mo_random_position IS INITIAL.
    ...
    DO mo_random_position->rand( ) TIMES.
      ...
      DATA(ls_position) = VALUE zdakm_ivit(
  ...
  METHOD if_oo_adt_classrun~main.
   Data ld_enum TYPE ZCAKM_IN_STAT_ENUM .
  ld_enum = ZCAKM_IN_STAT_ENUM-created.
  out->write( 'enum' ).
  ld_enum =  CONV #( 'C' ).

  DATA ld_simple TYPE ZCAKM_IN_STAT_ENUM.

  ld_simple = CONV #( 'C' ).
  out->write( ld_simple ).

  SELECT FROM ZCAKM_I_INHE
